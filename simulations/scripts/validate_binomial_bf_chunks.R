...
tolerance <- if (is.null(args[["tolerance"]])) 1e-10 else
    as.numeric(args[["tolerance"]])
output_file <- args[["output-file"]]

if (is.null(corpus_root) || !nzchar(corpus_root) ||
    !bf_type %in% c("all", "point", "direction") ||
    !is.finite(tolerance) || tolerance < 0) {
    stop("usage: Rscript simulations/scripts/validate_binomial_bf_chunks.R ",
         "--corpus-root <path> [--case-set production] ",
         "[--fixture-set fixture_id] [--bf-priors id1,id2] ",
         "[--bf-type all|point|direction] ",
         "[--chunk-id 1|all] [--tolerance 1e-10] ",
         "[--output-file <csv>]")
}
...
corpus_root <- bfpwr_sim_require_corpus_root(
    ...
    purpose = "binomial BF chunk validation")
...
if (!is.null(args[["bf-priors"]])) {
    ...
    if (!identical(fixture$family, "binomial")) {
        stop("--fixture-set must refer to a binomial fixture: ", fixture_set)
    }
...
if (length(selected) == 0) {
    stop("no binomial BF priors selected")
}
...
for (bf_prior in selected) {
    if (!identical(bf_prior$test_family, "binomial") ||
        !bf_prior$bf_type %in% c("point", "direction")) {
        stop("selected BF prior is not a supported binomial BF prior: ",
...
cat("validated cached binomial BF chunks against current package output\n")
