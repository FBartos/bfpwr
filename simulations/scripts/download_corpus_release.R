...
corpus_root <- if (is.null(args[["corpus-root"]])) {
    bfpwr_sim_default_corpus_root()
} else {
    args[["corpus-root"]]
}
download_dir <- if (is.null(args[["download-dir"]])) {
    file.path("simulations", "downloads", bfpwr_sim_corpus_release_tag())
} else {
    args[["download-dir"]]
}
...
cat("\nSet these environment variables when running fixture tests:\n")
cat("BFPWR_SIM_CORPUS=", corpus_root, "\n", sep = "")
cat("BFPWR_SIM_REPO=", normalizePath(".", winslash = "/", mustWork = FALSE),
    "\n", sep = "")
