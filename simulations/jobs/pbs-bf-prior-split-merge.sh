...
if [ -z "${BFPWR_MERGE_PLAN_FILE:-}" ]; then
  echo "BFPWR_MERGE_PLAN_FILE is required" >&2
  exit 2
fi

MERGE_PLAN_INDEX="${BFPWR_MERGE_PLAN_INDEX:-${PBS_ARRAY_INDEX:-1}}"
...
RUN_DIR="${SCRATCHDIR:-/tmp}/${USER:-fbartos}/bfpwr-bf-smerge-${PBS_JOBID:-manual}-${MERGE_PLAN_INDEX}"
mkdir -p "$RUN_DIR"
...
cd "$RUN_DIR"
Rscript simulations/scripts/merge_bf_prior_split_plan_chunk.R \
  --corpus-root "$BFPWR_SIM_CORPUS" \
  --merge-plan-index "$MERGE_PLAN_INDEX" \
  --split-plan-file "$BFPWR_SPLIT_PLAN_FILE" \
  --merge-plan-file "$BFPWR_MERGE_PLAN_FILE"

echo "completed BF split merge plan index $MERGE_PLAN_INDEX"
