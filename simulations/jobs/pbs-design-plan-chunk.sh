...
RUN_DIR="${SCRATCHDIR:-/tmp}/${USER:-fbartos}/bfpwr-${PBS_JOBID:-manual}-${PLAN_INDEX}"
mkdir -p "$RUN_DIR"
...
if [ "${BFPWR_RECORD_ENV:-false}" = "true" ] &&
  ...
  Rscript simulations/scripts/record_environment.R \
    --output-file "$BFPWR_SIM_CORPUS/environment/environment-${PBS_JOBID:-manual}-${PLAN_INDEX}.rds"
fi

LOCAL_CORPUS="$RUN_DIR/corpus"
Rscript simulations/scripts/run_design_plan_chunk.R \
  --corpus-root "$LOCAL_CORPUS" \
  --plan-index "$PLAN_INDEX"

cd "$LOCAL_CORPUS"
find designs -type f | while IFS= read -r rel; do
...
echo "completed design plan index $PLAN_INDEX"
