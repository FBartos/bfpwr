...
## Gated fixture-corpus test for sequential z BF01 simulations. It checks cached
## MC summaries against pbf01seq references. Related sources: BF01/power formulas
## in paper/bfssd.Rnw 354-606 and BFGSD sequential design; fixture rows are not
## literal manuscript examples.
...
bfpwr_fixture_expect_sequential_summary_integrity(
    fixture,
    label = "sequential z-bf01 fixture")
...
expect_equal(nrow(fixture$search_summary), 0L,
             info = "sequential z-bf01 search targets are disabled")
expect_true(all(c("start20-by10-looks02", "start20-by10-looks20",
            ...
            info = "sequential z-bf01 fixture includes all registered schedules")
...
representative_cases <- data.frame(
    bf_prior_id = c(
        ...
        "z-bf01-point-null0p2-pm0p5-psd0-on-z-dpoint-0p2-usd1-short",
    ...
    prior_form = c(
        "point", "normal", "point", "normal", "point", "shifted-point",
        "normal"
    ),
    look_grid_name = c(
        "short", "short", "short", "short", "short", "short", "long"
    ),
    schedule_id = rep("start20-by10-looks05", 7),
...
expect_true(any(representative_cases$prior_form == "point") &&
                any(representative_cases$prior_form == "normal"),
